document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderNfcOrders(orders) {
    ...
    nfcOrdersLiveBody.innerHTML = orders.map(function (order) {
      ...
      return '<tr class="nfc-live-row" data-nfc-order-id="' + order.id + '">' +
        ...
        '<td><span class="nfc-quantity-pill">' + formatDashboardNumber(order.quantity) + '</span><div class="subtle-handle">Items ' + escapeDashboardHtml(formatNfcMoney(Math.max(0,Number(order.amount)-Number(order.shippingCost)),order.currency)) + '</div><div class="subtle-handle">Delivery ' + escapeDashboardHtml(formatNfcMoney(order.shippingCost,order.currency)) + '</div><strong>' + escapeDashboardHtml(formatNfcMoney(order.amount,order.currency)) + ' total</strong></td>' +
  ...
  function setPlanAdminModal(open, plan) {
    ...
    if (plan) {
      ...
      planAdminForm.elements.storageLimitMb.value = plan.storageLimitMb;
  ...
  function renderSuperAdminWithdrawals(withdrawals) {
    ...
    withdrawalAdminBody.innerHTML = withdrawals.map(function (withdrawal) {
      ...
      if (["approved","processing","completed"].indexOf(status) !== -1) quickActions += '<button class="user-action-btn edit" type="button" data-live-withdrawal-action="receipt" data-withdrawal-id="' + withdrawal.id + '">' + (withdrawal.receiptAvailable ? "Replace receipt" : "Upload receipt") + '</button>' + (withdrawal.receiptAvailable ? '<button class="user-action-btn" type="button" data-live-withdrawal-action="view-receipt" data-withdrawal-id="' + withdrawal.id + '">View receipt</button>' : '');
  ...
  function renderAffiliatePartners(partners) {
    ...
    affiliatePartnerBody.innerHTML = partners.map(function (partner) {
      ...
      return '<tr>' +
        ...
        '<td><div class="affiliate-rule-cell"><strong>' + escapeDashboardHtml(transactionTypeLabel(partner.paymentMethod)) + '</strong><span>' + escapeDashboardHtml(partner.bankDetails && partner.bankDetails.bankName ? partner.bankDetails.bankName + " · " + (partner.bankDetails.accountHolder || "Account holder missing") : "Bank details required") + '</span></div></td>' +
  ...
  function setAffiliateModal(modal, form, titleElement, open, record, kind) {
    ...
    if (kind === "partner") {
      ...
      if (record) { var bank=record.bankDetails||{}; form.elements.userId.value=record.user.id; form.elements.referralCode.value=record.referralCode; form.elements.commissionType.value=record.commissionType; form.elements.commissionValue.value=record.commissionValue; form.elements.paymentMethod.value="bank_transfer"; form.elements.accountHolder.value=bank.accountHolder||""; form.elements.bankName.value=bank.bankName||""; form.elements.accountNumber.value=bank.accountNumber||""; form.elements.branch.value=bank.branch||""; form.elements.swiftCode.value=bank.swiftCode||""; form.elements.status.value=record.status; }
  ...
  async function submitAffiliateForm(form, basePath, idField, modal, kind) {
    ...
    if(kind==="partner") payload={userId:Number(fd.get("userId")),referralCode:fd.get("referralCode").trim(),commissionType:fd.get("commissionType"),commissionValue:Number(fd.get("commissionValue")),paymentMethod:"bank_transfer",bankDetails:{accountHolder:fd.get("accountHolder").trim(),bankName:fd.get("bankName").trim(),accountNumber:fd.get("accountNumber").trim(),branch:fd.get("branch").trim(),swiftCode:fd.get("swiftCode").trim()},status:fd.get("status")};
  ...
  function renderSuperAdminCoupons(coupons){
    ...
    Array.from(couponAdminBody.querySelectorAll("tr")).forEach(function(row,index){var statusCell=row.children[5],coupon=coupons[index];if(!statusCell||!coupon)return;var visibility=document.createElement("div");visibility.className="subtle-handle";visibility.textContent=coupon.isPublic?"Visible to users":"Private code";statusCell.appendChild(visibility);});
  }
  ...
  if (settingsTabButtons.length) {
    ...
    var requestedSettingsPanel = window.location.hash.replace(/^#/, "");
    var requestedSettingsButton = settingsTabButtons.find(function (button) { return button.getAttribute("data-settings-tab-target") === requestedSettingsPanel; });
    if (requestedSettingsButton) requestedSettingsButton.click();
  }
  ...
  renderActivity();
  if (!document.body.classList.contains("user-workspace")) renderNotifications();
  resetProfilePreview();
